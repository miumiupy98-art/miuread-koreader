...
- 从 5.4.0-beta.8 对应的干净 develop 基线重新实现 5.6 评论能力，不继承 5.6.0-beta.1 / beta.2 的实验性公共划线实现。
- 下载入口统一增加“纯净版 + 评论”：先完成纯净 EPUB，再把评论作为独立任务下载；评论失败、暂停或限流不会改变正文已完成状态。
- 阅读界面的“评论”中心直接增加评论状态、下载/更新当前章评论、补全本书评论和更新本书全部评论，不再把主要入口藏在深层插件设置。
- 修复“阅读评论”关闭后菜单仍显示“已开启”：开关保存后关闭并重建评论中心，同时输出明确的 ThoughtDisplay 状态日志。
- clean EPUB 的评论显示改为本地运行时虚线层：只读取真正存在评论的 review_groups；普通公共 underlines 只作为 readReviews 的协议索引，不进入显示层。
- 新增独立 comment-download.sqlite3，仅保存评论下载状态与断点；现有 thoughts.sqlite3、个人划线、个人想法、批注同步和原 notes 下载器保持原样。
- 已下载纯净版可直接补评论，不重新生成或修改 EPUB；旧纯净版缺少原始坐标源时只补取对应章节的原始 XHTML 用于定位。
- 评论任务支持当前章、本地已有章节范围、断点继续、异常空结果保护和正文任务优先；休眠时评论任务保守暂停并在恢复后继续，不接入 Kindle/Kobo 后台保活核心。
- 修复主页正文下载终态：正文完成、失败或取消时立即清除旧进度条状态；评论下载进度作为独立次级状态显示，不再覆盖正文“已下载”状态。
- 保留原“划线与想法版” span/a 生成链路，不在本版本重构 notes EPUB；独立 runtime 只服务纯净版。
