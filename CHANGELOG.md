...
- 评论正文解析恢复 5.3 已验证的数据结构：从 `pageReviews / reviews / updated` 读取评论列表，并兼容每条记录的 `review` 包装层，统一提取正文、作者、点赞、时间与 reviewId。
- Web 返回 range 但没有解析出真实评论时，不再把该 range 视为完成；未解析范围保持待补，并自动使用 Gateway 对这些范围重新请求。
- Web 与 Gateway 使用同一套评论解析器；只有实际解析到评论正文的 range 才能进入完成状态，0 条结果统一保持 `partial / parser_unknown`。
- 新增安全的评论响应结构诊断日志，仅记录顶层键、评论容器名称、数组/映射形态、条目数量、review 包装层数量、解析数量与异常数量，不记录评论正文、用户名、Cookie 或凭据。
- 兼容对象形态的评论列表：若 JSON 解码后评论容器不是标准数组但包含表对象，允许按对象值读取，避免因 Lua `ipairs` 得到 0 条而误判为空。
- 升级后再次一次性作废 5.5.0-beta.1 留下的 `0 评论 + complete=true` 状态；已有非空评论缓存继续保留，任何新的 0 条结果都不能覆盖。
- 本版暂时不自动确认“本章确实无评论”；真实空章节会保持待补，待评论解析链经过实机确认后再恢复无评论完成状态。
- 本版不修改正文 EPUB、locator、评论收藏、阅读进度、阅读时间、用户批注同步、Kindle/Kobo 电源管理、本地书、书架、封面和 OTA 核心。

## 5.5.0-beta.1

- 保留现有“唯一正文 + 独立评论缓存”架构，正文 EPUB、持久 locator 与 `thoughts.sqlite3` 继续相互独立。
- 新增独立评论获取器 `thought_fetcher.lua`，评论获取不再经过 beta.10～beta.13 逐步叠加的空结果判断链。
- 评论请求重新采用 Web `readReviews` 主路径；Web 请求失败或完全没有返回本次请求的 range 时，再使用 Gateway 回退。
- 评论结果直接按照 `bookId + chapterUid + range` 解析并保存，不再通过 Web 与 Gateway 相互验证“是否真的没有评论”。
- 请求的 range 未返回、返回结构无法识别或评论内容无法正常解析时，该 range 保持待补，不再记为“0 条评论且获取完成”。
- 已有评论缓存受到保护；刷新时出现异常 0 条结果不会清除本地已有评论。
- 正常评论获取继续直接复用正文下载阶段保存的 locator，不需要重新生成 EPUB；只有 locator 缺失或用户主动强制刷新时才重新获取划线定位。
- 评论删除、重新获取和刷新均不会影响正文、评论收藏、阅读记录及用户批注。
- 新增评论获取诊断日志，可分别看到 Web / Gateway 来源、请求 range、实际返回 range、评论数量及待补数量。
- 本版不修改正文生成、阅读进度、阅读时间、用户批注同步、Kindle/Kobo 电源管理、本地书扫描、微信书架、封面及 OTA 核心。
...
- 保留 beta.9 运行时评论划线、按需获取、下一章评论预加载、整本评论获取及缓存管理；旧 notes EPUB 的读取兼容、旧评论迁移和 `Annotations:apply()` 暂时保留，待后续 beta 验证稳定后再瘦身删除。

## 5.4.0-beta.9

- 新增纯净 EPUB 的按需评论实验链路：阅读中可直接获取或更新当前章节评论，评论写入每本书独立的 `thoughts.sqlite3`，无需重新下载或重新生成书籍。
- 新增运行时评论划线层：仅对当前章节中能够唯一、可靠匹配的评论原文建立临时 XPointer 与可点击下划线；重复或无法确认的位置直接跳过，绝不猜测错位。
- 运行时定位不写回 EPUB、不持久化 XPointer；字号、排版或正文变化后会重新映射，beta.8 原有“划线与想法版”注入链路继续保留作为本轮验证回退。
- 新增“下一章评论预加载”，与现有下一章正文预读取完全独立，按 `book_id + chapter_uid` 绑定目标；设备休眠、下载繁忙、阅读交互或运行压力较高时自动让路。
- 新增“获取整本评论”：已完整缓存章节自动跳过，部分结果保留并可再次补取；认证、限流、网络或连续异常时主动停止，避免后台持续请求。
- 评论中心整理为“评论显示 / 评论获取 / 评论收藏 / 评论缓存”四区，完整保留本章、本书、全部收藏与收藏搜索。
- 新增本章/本书评论缓存状态与删除入口；删除远端评论缓存不会删除任何已收藏评论。
- 评论缓存增加独立 fetch 状态，区分“尚未获取”“已缓存 0 条”“部分获取”“完整获取”；整本缓存统计改为单次 SQLite 连接，避免长书逐章打开数据库。
- 本版只建立新评论引擎，不切换主页下载模型：纯净版/划线与想法版、章节下载、旧评论迁移和 `Annotations:apply()` 暂时原样保留，待实机验证运行时定位后再在后续 beta 切换与瘦身。
- 保持 beta.8 的真实 Emoji、评论/划线总开关、评论收藏、Dialog 残影修复、Wi-Fi、下载、进度、阅读时间、锁屏及本地书链路不变。
