...
- 纯净 EPUB 支持阅读时按当前章节动态获取微信读书公共划线与评论，无需重新生成正文。
- 公共划线使用独立缓存和运行时显示层，不写入 KOReader 个人批注，也不进入个人批注同步链。
- 保留现有显示语义：普通公共划线为实线，有评论的位置为虚线；与用户自己的划线重叠时优先保留用户划线。
- 公共评论继续复用现有评论缓存与评论弹窗；评论或网络失败时保留旧缓存，不影响正文和个人批注。
- 仅当前章节按需获取；同章翻页不触发网络请求或全文重新定位。
