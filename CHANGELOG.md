...
- 基于 5.6 beta.1 之前的干净 develop 基线重新实现；不继承 beta.1 的 `public_annotation_*` 公共划线运行时、普通公共实线和相关缓存设计。
- 重新定义微信读书标记显示：普通公共划线只作为 `readReviews` 的 range 查询索引，不再显示；只有确实存在评论的 range 才允许显示评论虚线。新生成的“划线与想法版”也只注入真实评论 range；旧 EPUB 中已有的普通实线由阅读样式统一隐藏。
- 新增“纯净版 + 评论”：正文仍生成完全相同的纯净 EPUB，正文成功安装后再创建独立评论任务；评论失败、暂停或限流不会把正文状态改回未完成。
- 新增独立评论下载服务，复用现有 `Annotations:fetch_chapter()`、underlines → readReviews、range 对应、断点合并和 `thoughts.sqlite3`；不再创建第二套评论正文数据库。
- `thoughts.sqlite3` 增加轻量评论获取状态与任务状态表，用来区分 never / fetching / complete / empty / partial / failed / suspicious_zero 等状态；评论内容仍由原有 Thoughts 系统保存和弹出。
- 评论更新采用非破坏提交：完整结果才更新稳定快照；部分失败保留既有评论；已有非空评论遇到一次异常 0 结果时标记待确认，不自动清空。
- 纯净版阅读时新增本地评论 runtime：只读取已经独立下载并与当前 bookVersion 匹配的评论，复用原始章节 SourcePosition 和 AnnotationCoord 严格定位；重复文本无法唯一确认或定位失败时直接省略，不做百分比/数字位置猜测。
- “阅读评论”成为显示总闸门：关闭后 clean runtime 立即清空、不绘制、不响应点击，晚到回调也不能重新显示；关闭不会删除评论数据。划线与想法版继续沿用原有内部 anchor / Popup 兼容链。
- 评论菜单增加“下载/更新当前章评论”“补全本书评论”“更新本书全部评论”“重置独立评论下载状态”；书籍下载、范围下载和重新生成入口增加“纯净版 + 评论”。
- 下载管理新增独立评论任务及等待队列；正文下载优先于评论任务，评论任务暂停/熄屏时保留断点并重新排队。beta.2 暂不在普通翻页时自动联网更新评论。
- 主页区分正文状态与评论状态：正文下载完成时立即原子清除旧百分比；评论进度只作为“评论 xx%”次级状态显示，不再把已下载书籍误显示成正文仍在下载。完整主页刷新仍会从持久状态重新 reconcile。
- 用户自己的 KOReader 划线、想法、评论上传/删除/同步数据库与流程保持不变；公共评论 runtime 与本地批注点击重叠时，本地批注优先。
