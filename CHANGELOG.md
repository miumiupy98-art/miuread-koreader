...
- 纯净版阅读菜单新增“获取本章评论 / 更新本章评论”，无需重新下载带评论 EPUB。
- 当前章评论继续写入现有 `thoughts.sqlite3`；请求未完整完成时保留原缓存，不以失败结果覆盖已有评论。
- 纯净版评论位置复用现有微信章节坐标源与 `AnnotationCoord` 解析，不修改 EPUB 正文，也不写入 KOReader 用户划线。
- 新增独立运行时评论虚线：仅在能够唯一确认正文位置时显示；无法确认的评论不画线，避免错误对应。
- 点击运行时虚线继续使用现有评论弹窗、收藏与显示设置。
- 翻页、排版重算和旋转后重新计算当前页显示位置；关闭书籍时清理运行时评论层。
- 原有“划线与想法版”、用户划线、用户批注、批注同步、评论收藏、下载与 EPUB 生成逻辑保持不变。
