...
function Store:migrate()
    ...
    if schema<Config.SCHEMA then
        ...
        if schema<121 then
            logger.info("[MiuRead][Migration] schema 120 -> 121 begin","from=",tostring(schema))
            local current=self:preferences()
            current.thoughts=type(current.thoughts)=="table" and current.thoughts or {}
            if current.thoughts.prefetch_next==nil then current.thoughts.prefetch_next=true end
            self:save_preferences(current)
            logger.info("[MiuRead][Migration] schema 120 -> 121 done",
                "thought_prefetch_next=",tostring(current.thoughts.prefetch_next))
        end
        if schema<122 then
            logger.info("[MiuRead][Migration] schema 121 -> 122 begin","from=",tostring(schema))
            local current=self:preferences()
            current.thoughts=type(current.thoughts)=="table" and current.thoughts or {}
            if current.thoughts.initial_preload_count==nil then
                current.thoughts.initial_preload_count=tonumber(Config.THOUGHT_INITIAL_PRELOAD_DEFAULT) or 5
            end
            self:save_preferences(current)
            logger.info("[MiuRead][Migration] schema 121 -> 122 done",
                "thought_initial_preload_count=",tostring(current.thoughts.initial_preload_count))
        end
